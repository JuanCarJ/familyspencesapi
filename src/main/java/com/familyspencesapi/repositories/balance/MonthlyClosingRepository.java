...
public interface MonthlyClosingRepository extends JpaRepository<Closings, UUID> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(SUM(i.total), 0) FROM Income i WHERE i.family = :familyId AND (i.period = :period OR LOWER(i.period) = :monthName)")
    Double calculateMonthlyIncome(@org.springframework.data.repository.query.Param("familyId") UUID familyId, @org.springframework.data.repository.query.Param("period") String period, @org.springframework.data.repository.query.Param("monthName") String monthName);
}
