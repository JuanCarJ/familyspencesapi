...
public interface MonthlyClosingRepository extends JpaRepository<Closings, UUID> {
    ...
    long countByFamilyIdAndClosingDateBetween(UUID familyId, java.time.LocalDate startDate, java.time.LocalDate endDate);

    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(SUM(e.value), 0) FROM Expense e WHERE e.familyId = :familyId AND e.period = :period")
    java.math.BigDecimal calculateMonthlyExpenses(@org.springframework.data.repository.query.Param("familyId") UUID familyId, @org.springframework.data.repository.query.Param("period") String period);
